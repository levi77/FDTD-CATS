...
#define nx  400  //for visual comparison test 60
#define ny  400 //for visual comparison test 60	
...
{   
   ...
   for (i=0;i<nx;i++){
	...
	//hz2[nx / 2][ny / 2] = 1;
	hz2[nx / 2][ny / 2] = 1;
	for(int k=0; k<TOTAL_TIME;k+=time_block){
	for (int ii = 0; ii < TOTAL_TIME + nx; ii += s){	 
	  	for(t=k; t<(k+time_block); t++){


			for (int i = MAX(t+1, ii); i < MIN(t+nx, ii + s); i++) { //time-skew
				for (j=0; j<ny; j++){ 	//iterating over X field as CATS-1 paper says.
					ey2[i-t][j] = ey2[i-t][j] - 0.5*(hz2[i-t][j]-hz2[i-1-t][j]);
				}
			...
			for (int i = MAX(t, ii); i < MIN(t +nx, ii + s); i++) {
				for (j=1; j<ny; j++){ 	 	
				     ex2[i-t][j] = ex2[i-t][j] - 0.5*(hz2[i-t][j]-hz2[i-t][j-1]);
			    }
	    	}
			...
			for (int i = MAX(t, ii-1); i < MIN(t +nx, ii-1 + s); i++) { 	
				for (j=0; j<ny; j++){
					hz2[i - t][j] = hz2[i - t][j] - 0.5*(ex2[i - t][j + 1] - ex2[i - t][j] + ey2[i + 1 - t][j] - ey2[i - t][j]);
